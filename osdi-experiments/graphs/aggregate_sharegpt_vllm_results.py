...
# python osdi-experiments/graphs/aggregate_sharegpt_vllm_results.py

REPO = Path("/home/sabiha/sarathi-serve")
TIMES_CSV = Path("/home/sabiha/sarathi-serve/log/time_sharegpt_vllm.csv")
OUTPUT_ROOT = REPO / "benchmark_output" / "sharegpt"
OUT_CSV = REPO / "osdi-experiments" / "graphs" / "sharegpt_vllm_full_results.csv"


def get_run_dir(model: str, qps: str) -> Path:
    run_name = f"sharegpt_vllm_poison_{qps}_qps_{model}"
...
def main():
    ...
    for _, r in times_df.iterrows():
        ...
        rows.append(row)
        print(f"[OK] {model} / qps={qps}")

    fieldnames = [
        "model", "qps", "inference_time",
