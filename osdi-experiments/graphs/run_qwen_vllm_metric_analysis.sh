...
# chmod +x osdi-experiments/graphs/run_qwen_vllm_metric_analysis.sh
# ./osdi-experiments/graphs/run_qwen_vllm_metric_analysis.sh

REPO="/home/sabiha/sarathi-serve"
ANALYZER="$REPO/osdi-experiments/graphs/analyze_sarathi_metric.py"
TIMES_CSV="$REPO/osdi-experiments/graphs/qwen_vllm_times.csv"
OUTPUT_ROOT="$REPO/benchmark_output/figure-1"
SUMMARY="$REPO/osdi-experiments/graphs/qwen_vllm_metric_analysis_summary.csv"
...

{
    ...
    while IFS=',' read -r qps total_time || [[ -n "$qps" ]]; do
        RUN_DIR="$OUTPUT_ROOT/arxiv_vllm_poison_${qps}_qps_qwen"
