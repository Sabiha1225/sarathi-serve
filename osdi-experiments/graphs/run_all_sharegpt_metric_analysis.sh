...
# chmod +x osdi-experiments/graphs/run_all_sharegpt_metric_analysis.sh
# ./osdi-experiments/graphs/run_all_sharegpt_metric_analysis.sh

REPO="/home/sabiha/sarathi-serve"
ANALYZER="$REPO/osdi-experiments/graphs/analyze_sarathi_metric.py"
TIMES_CSV="$REPO/osdi-experiments/graphs/time_sharegpt_sarathi.csv"
OUTPUT_ROOT="$REPO/benchmark_output/sharegpt"
SUMMARY="$REPO/osdi-experiments/graphs/sharegpt_metric_analysis_summary.csv"
...

{
    ...
    while IFS=',' read -r dataset model chunk_size inference requests request_arrival || [[ -n "$dataset" ]]; do
        ...
        RUN_DIR="$OUTPUT_ROOT/sharegpt_chunk_${chunk_size}_poison_${qps}_qps_${model}"
