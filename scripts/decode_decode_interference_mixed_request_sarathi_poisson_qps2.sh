...
# chmod +x scripts/decode_decode_interference_mixed_request_sarathi_poisson_qps2.sh
# ./scripts/decode_decode_interference_mixed_request_sarathi_poisson_qps2.sh
# nohup ./scripts/decode_decode_interference_mixed_request_sarathi_poisson_qps2.sh \
#  > log/decode_decode_interference_mixed_request_sarathi_poisson_qps2.log 2>&1 &
...
SUMMARY="$OUTPUT_ROOT/mixed_request_sarathi_poisson_qps2_summary.csv"
CONFIG_YML="decode_decode_interference_mixed_request_sarathi.yml"
...
for model_pair in "${MODELS[@]}"; do
    ...
    echo ""
    echo "============================================================"
    echo "Running model=$model_safe scheduler=sarathi request_type=mixed_poisson_qps2"
    echo "============================================================"

    echo "" >> "$TIME_FILE"
    echo "model=$model_safe scheduler=sarathi request_type=mixed interval_provider=poisson qps=$QPS" >> "$TIME_FILE"
    ...
    if [[ -z "$GENERATED_DIR" || ! -d "$GENERATED_DIR" ]]; then
        ...
        echo "$model_safe,sarathi,mixed_poisson_qps2,failed,$inference_time,$wall_time,$FAILED_DIR" >> "$SUMMARY"
        rm -f "$MARKER"
    ...
    if [[ "$run_status" -eq 0 ]]; then
        ...
        echo "$model_safe,sarathi,mixed_poisson_qps2,success,$inference_time,$wall_time,$FINAL_DIR" >> "$SUMMARY"
...
echo ""
echo "Mixed-request sarathi (poisson qps=2) run complete."
