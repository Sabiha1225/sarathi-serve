#!/usr/bin/env bash
set -uo pipefail
# chmod +x scripts/run_yi_gpu_preemption_sweep.sh
# nohup ./scripts/run_yi_gpu_preemption_sweep.sh \
#  > log/yi_gpu_preemption_sweep.log 2>&1 &

# echo $! > log/yi_gpu_preemption_sweep.pid
...
for gpu_util in "${GPU_VALUES[@]}"; do
    ...
    RUN_NAME="arxiv_sarathi_gpu_${gpu_label}_yi"
