...
# chmod +x scripts/decode_decode_interference_mixed_request_sarathi.sh

# nohup ./scripts/decode_decode_interference_mixed_request_sarathi.sh \
#  > log/decode_decode_interference_mixed_request_sarathi.log 2>&1 &
...
SUMMARY="$OUTPUT_ROOT/mixed_request_sarathi_summary.csv"
...
for model_pair in "${MODELS[@]}"; do
    ...
    echo ""
    echo "============================================================"
    echo "Running model=$model_safe scheduler=sarathi request_type=mixed"
    echo "============================================================"

    echo "" >> "$TIME_FILE"
    echo "model=$model_safe scheduler=sarathi request_type=mixed" >> "$TIME_FILE"
    ...
    if [[ -z "$GENERATED_DIR" || ! -d "$GENERATED_DIR" ]]; then
        ...
        echo "$model_safe,sarathi,mixed,failed,$inference_time,$wall_time,$FAILED_DIR" >> "$SUMMARY"
        rm -f "$MARKER"
    ...
    if [[ "$run_status" -eq 0 ]]; then
        ...
        echo "$model_safe,sarathi,mixed,success,$inference_time,$wall_time,$FINAL_DIR" >> "$SUMMARY"
...
echo ""
echo "Mixed-request sarathi run complete."
