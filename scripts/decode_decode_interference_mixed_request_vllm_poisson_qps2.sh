...
# chmod +x scripts/decode_decode_interference_mixed_request_vllm_poisson_qps2.sh
# ./scripts/decode_decode_interference_mixed_request_vllm_poisson_qps2.sh

# nohup ./scripts/decode_decode_interference_mixed_request_vllm_poisson_qps2.sh \
#  > log/decode_decode_interference_mixed_request_vllm_poisson_qps2.log 2>&1 &

REPO="/home/sabiha/sarathi-serve"
MAIN="$REPO/sarathi/benchmark/main.py"
CONSTANTS_FILE="$REPO/sarathi/benchmark/constants.py"
OUTPUT_ROOT="$REPO/benchmark_output/vllm_decode_decode_interference"
TIME_FILE="$REPO/log/decode_decode_interference.txt"
LATEST_LOG="$REPO/log/vllm_log.log"
SUMMARY="$OUTPUT_ROOT/mixed_request_vllm_poisson_qps2_summary.csv"
CONFIG_YML="decode_decode_interference_mixed_request_vllm.yml"
...
if [[ ! -f "$SUMMARY" ]]; then
    echo "model,scheduler,request_type,status,inference_time_sec,wall_time_sec,output_dir" > "$SUMMARY"
fi
...
for model_pair in "${MODELS[@]}"; do
    ...
    RUN_NAME="${model_safe}_mixed_req_poisson_qps2"
    ...
    echo ""
    echo "============================================================"
    echo "Running model=$model_safe scheduler=vllm request_type=mixed_poisson_qps2"
    echo "============================================================"

    echo "" >> "$TIME_FILE"
    echo "model=$model_safe scheduler=vllm request_type=mixed interval_provider=poisson qps=$QPS" >> "$TIME_FILE"
    ...
    if [[ -z "$GENERATED_DIR" || ! -d "$GENERATED_DIR" ]]; then
        ...
        mv "$LOG_TMP" "$FAILED_DIR/vllm_log.log"
        echo "$model_safe,vllm,mixed_poisson_qps2,failed,$inference_time,$wall_time,$FAILED_DIR" >> "$SUMMARY"
        rm -f "$MARKER"
    ...
    if [[ "$run_status" -eq 0 ]]; then
        ...
        mv "$LOG_TMP" "$GENERATED_DIR/vllm_log.log"
        mv "$GENERATED_DIR" "$FINAL_DIR"
        cp "$FINAL_DIR/vllm_log.log" "$LATEST_LOG"
        echo "$model_safe,vllm,mixed_poisson_qps2,success,$inference_time,$wall_time,$FINAL_DIR" >> "$SUMMARY"
...
echo ""
echo "Mixed-request vllm (poisson qps=2) run complete."
