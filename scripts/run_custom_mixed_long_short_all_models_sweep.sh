...
# chmod +x scripts/run_custom_mixed_long_short_all_models_sweep.sh
# nohup ./scripts/run_custom_mixed_long_short_all_models_sweep.sh > log/custom_mixed_long_short_all_models_sweep.log 2>&1 &
# echo $! > log/custom_mixed_long_short_all_models_sweep.pid
# tail -f log/custom_mixed_long_short_all_models_sweep.log
...
SUMMARY="$OUTPUT_ROOT/custom_mixed_long_short_all_models_sweep_summary.csv"

DATASET_LABEL="custom_sarathi_mixed_long_short_output"
...
run_one() {
    ...
    echo "" >> "$TIME_FILE"
    echo "Custom mixed long-short $sweep_type: model=$model_name num_requests=$num_requests qps=$qps" >> "$TIME_FILE"
