...
# python scripts/varried_chunk_graph_generation_arxiv_tp_pp.py
...
def parse_sweep(path):
    """Tidy layout: Sarathi block in columns 1-13, vLLM block in columns 17-28,
    one row per (model, chunk_size, qps) for Sarathi and one row per (model, qps)
    for vLLM (chunk_size doesn't apply there)."""
    ...
    sarathi = sarathi[sarathi["model"] != "model"]
...
def chart_metric_vs_chunk_by_qps(df, metric, ylabel, title_metric, out_path, label, log_y=False):
    ...
    fig.suptitle(f"{title_metric} vs chunk size, by QPS — {label}\n"
...
RUNS = [
    dict(
        csv="/home/sabiha/sarathi-serve/varried_chunk/Inference_Results(Sarathi_Serve)_arxiv_TP1_PP2.csv",
        out_dir="/home/sabiha/sarathi-serve/varried_chunk/arxiv_tp1_pp2",
        label="Arxiv, TP=1 PP=2",
        suffix="arxiv_tp1_pp2",
    ),
    dict(
        csv="/home/sabiha/sarathi-serve/varried_chunk/Inference_Results(Sarathi_Serve)_arxiv_TP1_PP1.csv",
        out_dir="/home/sabiha/sarathi-serve/varried_chunk/arxiv_tp1_pp1",
        label="Arxiv, TP=1 PP=1",
        suffix="arxiv_tp1_pp1",
    ),
]
...
if __name__ == "__main__":
    for run in RUNS:
        ...
        df = parse_sweep(run["csv"])

        for metric, ylabel, title_metric, file_suffix, log_y in METRICS:
            chart_metric_vs_chunk_by_qps(
                df, metric, ylabel, title_metric,
                out_path=out_dir / f"{file_suffix}_vs_chunk_by_qps_{run['suffix']}.png",
                label=run["label"], log_y=log_y,
            )

        print(f"done: {run['csv']} -> {out_dir}")
