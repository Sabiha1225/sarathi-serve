...
# chmod +x scripts/run_mixtral_sweep.sh
# ./scripts/run_mixtral_sweep.sh

# Use nohup so it continues after SSH/laptop disconnects:
# nohup ./scripts/run_mixtral_sweep.sh \
#  > log/mixtral_sweep.log 2>&1 &

# echo $! > log/mixtral_sweep.pid
# tail -f log/mixtral_sweep.log      Check progress:
# ps -fp "$(cat log/mixtral_sweep.pid)"      ps -fp "$(cat log/mixtral_sweep.pid)"
# kill "$(cat log/mixtral_sweep.pid)"          kill "$(cat log/mixtral_sweep.pid)"
# watch -n 10 'ls -lt benchmark_output/figure-1 | head'  To monitor output directories:
# tmux new -s mixtral-sweep       Using tmux is even more convenient for reconnecting:
# ./scripts/run_mixtral_sweep.sh
# Detach with Ctrl+B, then D. Reconnect later:
# tmux attach -t mixtral-sweep
...
SUMMARY="$OUTPUT_ROOT/arxiv_mixtral_qps_chunk_sweep.csv"
...
for qps in "${QPS_VALUES[@]}"; do
    ...
    for chunk_size in "${CHUNK_SIZES[@]}"; do
        RUN_NAME="arxiv_chunk_${chunk_size}_poison_${qps}_qps_mixtral"
