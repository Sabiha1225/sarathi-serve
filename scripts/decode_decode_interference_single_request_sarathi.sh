...
# chmod +x scripts/decode_decode_interference_single_request_sarathi.sh
# ./scripts/decode_decode_interference_single_request_sarathi.sh

# nohup ./scripts/decode_decode_interference_single_request_sarathi.sh \
#  > log/decode_decode_interference_single_request_sarathi.log 2>&1 &
...
SUMMARY="$OUTPUT_ROOT/single_request_sarathi_summary.csv"
CONFIG_YML="decode_decode_interference_single_request_sarathi.yml"
...
for model_pair in "${MODELS[@]}"; do
    ...
    for rep in $(seq 1 "$NUM_REPS"); do
        ...
        echo ""
        echo "============================================================"
        echo "Running model=$model_safe scheduler=sarathi request_type=single rep=$rep"
        echo "============================================================"

        echo "" >> "$TIME_FILE"
        echo "model=$model_safe scheduler=sarathi request_type=single rep=$rep" >> "$TIME_FILE"
        ...
        if [[ -z "$GENERATED_DIR" || ! -d "$GENERATED_DIR" ]]; then
            ...
            echo "$model_safe,sarathi,single,$rep,failed,$inference_time,$wall_time,$FAILED_DIR" >> "$SUMMARY"
            rm -f "$MARKER"
        ...
        if [[ "$run_status" -eq 0 ]]; then
            ...
            echo "$model_safe,sarathi,single,$rep,success,$inference_time,$wall_time,$FINAL_DIR" >> "$SUMMARY"
...
echo ""
echo "Single-request sarathi sweep complete."
