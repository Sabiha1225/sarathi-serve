#!/usr/bin/env bash
set -uo pipefail
# chmod +x scripts/run_qwen_gpu_preemption_sweep.sh
# nohup ./scripts/run_qwen_gpu_preemption_sweep.sh \
#  > log/qwen_gpu_preemption_sweep.log 2>&1 &

# echo $! > log/qwen_gpu_preemption_sweep.pid
...
for gpu_util in "${GPU_VALUES[@]}"; do
    ...
    RUN_NAME="arxiv_sarathi_gpu_${gpu_label}_qwen"
