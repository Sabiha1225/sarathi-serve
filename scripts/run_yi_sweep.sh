...
# chmod +x scripts/run_yi_sweep.sh
# ./scripts/run_yi_sweep.sh

# Use nohup so it continues after SSH/laptop disconnects:
# nohup ./scripts/run_yi_sweep.sh \
#  > log/yi_sweep.log 2>&1 &

# echo $! > log/yi_sweep.pid
# tail -f log/yi_sweep.log      Check progress:
# ps -fp "$(cat log/yi_sweep.pid)"      ps -fp "$(cat log/yi_sweep.pid)"
# kill "$(cat log/yi_sweep.pid)"          kill "$(cat log/yi_sweep.pid)"
# watch -n 10 'ls -lt benchmark_output/figure-1 | head'  To monitor output directories:
# tmux new -s yi-sweep       Using tmux is even more convenient for reconnecting:
# ./scripts/run_yi_sweep.sh
# Detach with Ctrl+B, then D. Reconnect later:
# tmux attach -t yi-sweep
...
SUMMARY="$OUTPUT_ROOT/arxiv_yi_qps_chunk_sweep.csv"
...
for qps in "${QPS_VALUES[@]}"; do
    ...
    for chunk_size in "${CHUNK_SIZES[@]}"; do
        RUN_NAME="arxiv_chunk_${chunk_size}_poison_${qps}_qps_yi"
