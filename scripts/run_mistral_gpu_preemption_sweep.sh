#!/usr/bin/env bash
set -uo pipefail
# chmod +x scripts/run_mistral_gpu_preemption_sweep.sh
# nohup ./scripts/run_mistral_gpu_preemption_sweep.sh \
#  > log/mistral_gpu_preemption_sweep.log 2>&1 &

# echo $! > log/mistral_gpu_preemption_sweep.pid
...
for gpu_util in "${GPU_VALUES[@]}"; do
    ...
    RUN_NAME="arxiv_sarathi_gpu_${gpu_label}_mistral"
