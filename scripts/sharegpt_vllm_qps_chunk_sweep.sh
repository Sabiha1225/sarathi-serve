...
# chmod +x scripts/sharegpt_vllm_qps_chunk_sweep.sh
# ./scripts/sharegpt_vllm_qps_chunk_sweep.sh

# Use nohup so it continues after SSH/laptop disconnects:
# nohup ./scripts/sharegpt_vllm_qps_chunk_sweep.sh \
#  > log/sharegpt_vllm_qps_chunk_sweep.log 2>&1 &
...
SUMMARY="$OUTPUT_ROOT/sharegpt_vllm_qps_sweep_summary.csv"
CONFIG_YML="sharegpt_vllm_varied_chunk_size_varried_req_time.yml"
...
# Point constants.py's DEFAULT_CONFIG_FILE at the sharegpt vllm yml, commenting
...
for model_pair in "${MODELS[@]}"; do
    ...
    echo "" >> "$TIME_FILE"
    echo "# model=$model_safe vLLM sharegpt sweep" >> "$TIME_FILE"

    for qps in "${QPS_VALUES[@]}"; do
        RUN_NAME="sharegpt_vllm_poison_${qps}_qps_${model_safe}"
