...
# chmod +x scripts/decode_decode_interference_mixed_request_short_vllm.sh
# ./scripts/decode_decode_interference_mixed_request_short_vllm.sh

# nohup ./scripts/decode_decode_interference_mixed_request_short_vllm.sh \
#  > log/decode_decode_interference_mixed_request_short_vllm.log 2>&1 &
...
SUMMARY="$OUTPUT_ROOT/mixed_request_short_vllm_summary.csv"
CONFIG_YML="decode_decode_interference_mixed_request_short_vllm.yml"
...
for model_pair in "${MODELS[@]}"; do
    ...
    for mode_pair in "${ARRIVAL_MODES[@]}"; do
        ...
        echo ""
        echo "============================================================"
        echo "Running model=$model_safe scheduler=vllm request_type=mixed_short arrival=$mode_label"
        echo "============================================================"

        echo "" >> "$TIME_FILE"
        echo "model=$model_safe scheduler=vllm request_type=mixed_short arrival_mode=$mode_label" >> "$TIME_FILE"
        ...
        if [[ -z "$GENERATED_DIR" || ! -d "$GENERATED_DIR" ]]; then
            ...
            echo "$model_safe,vllm,mixed_short,$mode_label,failed,$inference_time,$wall_time,$FAILED_DIR" >> "$SUMMARY"
            rm -f "$MARKER"
        ...
        if [[ "$run_status" -eq 0 ]]; then
            ...
            echo "$model_safe,vllm,mixed_short,$mode_label,success,$inference_time,$wall_time,$FINAL_DIR" >> "$SUMMARY"
...
echo ""
echo "Mixed-request (short, static + poisson qps=2) vllm run complete."
