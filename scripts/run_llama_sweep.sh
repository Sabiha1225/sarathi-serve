...
# chmod +x scripts/run_llama_sweep.sh
# ./scripts/run_llama_sweep.sh

# Use nohup so it continues after SSH/laptop disconnects:
# nohup ./scripts/run_llama_sweep.sh \
#  > log/llama_sweep.log 2>&1 &

# echo $! > log/llama_sweep.pid
# tail -f log/llama_sweep.log      Check progress:
# ps -fp "$(cat log/llama_sweep.pid)"      ps -fp "$(cat log/llama_sweep.pid)"
# kill "$(cat log/llama_sweep.pid)"          kill "$(cat log/llama_sweep.pid)"
# watch -n 10 'ls -lt benchmark_output/figure-1 | head'  To monitor output directories:
# tmux new -s llama-sweep       Using tmux is even more convenient for reconnecting:
# ./scripts/run_llama_sweep.sh
# Detach with Ctrl+B, then D. Reconnect later:
# tmux attach -t llama-sweep
...
SUMMARY="$OUTPUT_ROOT/arxiv_llama_qps_chunk_sweep.csv"
...
for qps in "${QPS_VALUES[@]}"; do
    ...
    for chunk_size in "${CHUNK_SIZES[@]}"; do
        RUN_NAME="arxiv_chunk_${chunk_size}_poison_${qps}_qps_llama"
