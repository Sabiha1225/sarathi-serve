...
# chmod +x scripts/sharegpt_vllm_gpu_sweep.sh
# nohup ./scripts/sharegpt_vllm_gpu_sweep.sh \
#  > log/sharegpt_vllm_gpu_sweep.log 2>&1 &
...
for model_pair in "${MODELS[@]}"; do
    ...
    for gpu_util in "${GPU_VALUES[@]}"; do
        ...
        RUN_NAME="sharegpt_vllm_gpu_${gpu_label}_${model_safe}"
        ...
        echo
        echo "============================================================"
        echo "Running vLLM: model=$model_safe GPU=$gpu_util, QPS=$QPS"
        ...
        {
            echo
            echo "vLLM sharegpt : model=$model_safe 200 Requests - gpu = $display_gpu , 0 Preemption"
        } >> "$TIME_FILE"
...
echo
echo "vLLM GPU-memory-utilization sweep completed for all models."
