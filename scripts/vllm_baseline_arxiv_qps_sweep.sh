...
# chmod +x scripts/vllm_baseline_arxiv_qps_sweep.sh
# ./scripts/vllm_baseline_arxiv_qps_sweep.sh

# Use nohup so it continues after SSH/laptop disconnects:
# nohup ./scripts/vllm_baseline_arxiv_qps_sweep.sh \
#  > log/vllm_baseline.log 2>&1 &


REPO="/home/sabiha/sarathi-serve"
MAIN="$REPO/sarathi/benchmark/main.py"
CONSTANTS_FILE="$REPO/sarathi/benchmark/constants.py"
OUTPUT_ROOT="$REPO/benchmark_output/vllm_baseline"
TIME_FILE="$REPO/log/vllm_baseline.txt"
LATEST_LOG="$REPO/log/sarathi_log.log"
SUMMARY="$OUTPUT_ROOT/vllm_baseline_summary.csv"
CONFIG_YML="arxiv_vllm_varied_chunk_size_varried_req_time.yml"
...
if [[ ! -f "$SUMMARY" ]]; then
    echo "model,qps,status,inference_time_sec,wall_time_sec,output_dir" \
        > "$SUMMARY"
fi

# Point constants.py's DEFAULT_CONFIG_FILE at the arxiv vllm yml, commenting
# out whatever line is currently active. This is the only way to select a
# config file, since main.py takes no --config_file flag. Model switching
# itself is done purely via --model_name below, so this only needs to run once.
set_active_config() {
    local yml_name="$1"
...
for model_pair in "${MODELS[@]}"; do
    ...
    echo "" >> "$TIME_FILE"
    echo "# model=$model_safe vLLM arxiv sweep" >> "$TIME_FILE"

    for qps in "${QPS_VALUES[@]}"; do
        RUN_NAME="arxiv_vllm_poison_${qps}_qps_${model_safe}"
        ...
        echo ""
        echo "============================================================"
        echo "Running vLLM: model=$model_safe QPS=$qps"
        echo "============================================================"

        echo "# vLLM Poisson $qps request per seconds output=$RUN_NAME" >> "$TIME_FILE"
        ...
        if [[ -z "$GENERATED_DIR" || ! -d "$GENERATED_DIR" ]]; then
            ...
            echo "$model_safe,$qps,failed,$inference_time,$wall_time,$FAILED_DIR" \
                >> "$SUMMARY"
        ...
        if [[ "$run_status" -eq 0 ]]; then
            ...
            echo "$model_safe,$qps,success,$inference_time,$wall_time,$FINAL_DIR" \
                >> "$SUMMARY"
