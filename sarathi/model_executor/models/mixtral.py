...
class MixtralDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # DiscoResearch/mixtral-7b-8expert start
        num_experts = getattr(
        ...
        if num_experts is None:
            raise ValueError("MixtralConfig must define num_local_experts or num_experts.")

        if num_experts_per_tok is None:
            raise ValueError(
                "MixtralConfig must define num_experts_per_tok or num_experts_per_token."
            )

        # DiscoResearch/mixtral-7b-8expert end
...
class MixtralModel(nn.Module):
    ...
    def forward(
        ...
        for i in range(len(self.layers)):
            ...
            # )
            hidden_states = layer(
                positions, hidden_states, kv_caches[i]
            )
