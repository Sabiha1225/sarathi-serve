...
for the poisson-arrival (qps=2) mixed-request runs, across both schedulers and
all three models, into one CSV:
  decode_decode_mixed_request_poisson_qps2_fields.csv  (19 requests x 3 models x 2 schedulers)

python3 decode-decode-interference/collect_ttft_tpot_fields_poisson_qps2.py
"""
...
for scheduler, root in SCHEDULERS.items():
    for model in MODELS:
        run_dir = root / f"{model}_mixed_req_poisson_qps2"
        df = load_run(run_dir)
...
out_path = REPO / "decode-decode-interference" / "decode_decode_mixed_request_poisson_qps2_fields.csv"
mixed_df.to_csv(out_path, index=False)
print(f"wrote {out_path} ({len(mixed_df)} rows)")
